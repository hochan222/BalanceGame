  ...
  return (
    <>
      <VStack w="100%">
        ...
        <Grid templateColumns="1fr 1fr" gap={1} h="max-content" minH={180} w="100%">
            ...
            >
              <VStack w="full">
                {article.leftItem?.split('\n').map((text) => {
                  return (
                    <Text fontSize="md" color="red.500" w="full" align="center">
                      {text}
                    </Text>
                  );
                })}
                {voted && (
                  <Text fontSize="2xl" color="red.500" w="full" align="center">
                    {article.leftCount + leftCount <= 0 ? 0 : article.leftCount + leftCount}표
                  </Text>
                )}
                {selectedVote === 'left' && <CheckIcon color="green.500" />}
              </VStack>
            </Container>
          </GridItem>
          <GridItem overflowX="auto">
            ...
            >
              <VStack w="full">
                {article.rightItem?.split('\n').map((text) => {
                  return (
                    <Text fontSize="md" color="blue.500" w="full" align="center">
                      {text}
                    </Text>
                  );
                })}
                {voted && (
                  <Text fontSize="2xl" color="blue.500" w="full" align="center">
                    {article.rightCount + rightCount <= 0 ? 0 : article.rightCount + rightCount}표
                  </Text>
                )}
                {selectedVote === 'right' && <CheckIcon color="green.500" />}
              </VStack>
            </Container>
          </GridItem>
        </Grid>
